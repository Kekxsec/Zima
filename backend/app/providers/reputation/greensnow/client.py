# backend/app/providers/reputation/greensnow/client.py
from __future__ import annotations

# --- Migration notes (severity/inheritance stripped) ---
# STRIPPED: severity=FindingSeverity.HIGH
# --- End migration notes ---
# Extracted/adapted from SpiderFoot module: modules/sfp_greensnow.py (MIT licensed)
...
class GreensnowProvider(BaseProviderClient):
    name = "greensnow"
        ...
        for ip_value in targets:
            ...
            findings.append(
                dict(
                    provider=self.name,
                    category="reputation",
                    title="IP listed in Greensnow",
                    description=f"{ip_value} appears in Greensnow blocklist feed",
                    entity_type="ip_address",
                    entity_value=ip_value,
                    confidence=0.77,
                    tags=["reputation", "greensnow", "malicious", "passive"],
                )
            )
            evidence.append(
                dict(
                    source=self.name,
                    description=f"Greensnow feed lookup for {ip_value}",
    ...
    async def _fetch_feed(self) -> set[str]:
        ...
        try:
            ...
            raise ProviderError(
                message="Greensnow feed request failed",
                retryable=True,
            ) from exc
        if response.status_code == 429:
            raise ProviderError(
                message="Greensnow feed rate limited request",
                retryable=True,
            )
        if response.status_code >= 500:
            raise ProviderError(
                message="Greensnow feed upstream unavailable",
                retryable=True,
                status_code=response.status_code,
            )
        if response.status_code != 200:
            raise ProviderError(
                message="Greensnow feed returned unexpected response",
                retryable=False,
                status_code=response.status_code,
            )
        if not response.content.strip():
            raise ProviderError(
                message="Greensnow feed returned empty response body",
                retryable=False,
            )
        ips: set[str] = set()
    ...
    def _validate_ipv4(value: str) -> str:
        try:
            ...
            raise ProviderError(
                message=f"Unsupported input for Greensnow: {value}",
                retryable=False,
            ) from exc
        if parsed.version != 4:
            raise ProviderError(
                message=f"Unsupported input for Greensnow (IPv4 only): {value}",
                retryable=False,
            )
        return str(parsed)
    ...
    def _expand_netblock(self, netblock: str) -> list[str]:
        try:
            ...
            raise ProviderError(
                message=f"Unsupported netblock for Greensnow: {netblock}",
                retryable=False,
            ) from exc
        if network.version != 4:
            raise ProviderError(
                message=f"Unsupported netblock for Greensnow (IPv4 only): {netblock}",
                retryable=False,
            )
        max_prefix = 24
