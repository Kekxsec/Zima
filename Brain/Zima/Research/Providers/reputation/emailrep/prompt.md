---
title: "prompt / reputation / emailrep"
aliases: ["emailrep", "emailrep prompt", "emailrep research prompt"]
tags: [zima, research, prompts, provider-research, reputation, emailrep, graph_exclude]
...
- Provider name: emailrep
- Provider category: reputation
- Provider website / API docs URL: https://emailrep.io/
- Target module(s) from my provider map: username_exposure, account_enumeration_risk
- Build priority / tier: 1 / Core
- Provider role: signal_producer
- Schema hints already captured in my notes:
  - likely provider method(s): get_reputation(email)
  - likely provider-side category label(s): email_reputation
  - likely supported entity type(s): email
  - rough provider confidence note from my research scaffold: 0.80
- Important provider-specific cautions:
- Use emailrep.io as primary source and distinguish reputation/enrichment traits from actual risk findings. Many response fields may support info/low signals only.
- Focus on documented fields like suspicious, references, details, and known source flags rather than the marketing summary.
- Do not treat a low reputation score as equivalent to confirmed compromise.
