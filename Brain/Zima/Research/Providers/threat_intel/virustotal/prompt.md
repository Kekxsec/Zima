---
title: "prompt / threat_intel / virustotal"
aliases: ["virustotal", "virustotal prompt", "virustotal research prompt"]
tags: [zima, research, prompts, provider-research, threat_intel, virustotal, graph_exclude]
...
- Provider category: threat_intel
- Provider website / API docs URL: https://docs.virustotal.com/reference/overview
- Target module(s) from my provider map: malware_ioc_monitor, domain_ioc_match
- Build priority / tier: 3 / Pro
- Provider role: signal_producer
- Schema hints already captured in my notes:
  - likely provider method(s): get_report(domain, ip, url)
  - likely provider-side category label(s): malware_detection
  - likely supported entity type(s): domain, ip_address, url
  - rough provider confidence note from my research scaffold: 0.90
...
- Likely reason: severity should vary by malicious vendor count, object type, and whether the result is phishing, malware, or weak reputation only.
- Status: document exact field-based thresholds from official schemas/examples.
