---
title: "prompt / breach / breachdirectory"
aliases: ["breachdirectory", "breachdirectory prompt", "breachdirectory research prompt"]
tags: [zima, research, prompts, provider-research, breach, breachdirectory, graph_exclude]
...
- Provider name: breachdirectory
- Provider category: breach
- Provider website / API docs URL: https://breachdirectory.org/
- Target module(s) from my provider map: breach_monitor
- Build priority / tier: 1 / Core
- Provider role: signal_producer
- Schema hints already captured in my notes:
  - likely provider method(s): search_breaches(email)
  - likely provider-side category label(s): credential_leak
  - likely supported entity type(s): email
  - rough provider confidence note from my research scaffold: 0.75-0.90
- Important provider-specific cautions:
- If no public official API docs exist, explicitly mark schema details as inferred-from-examples or unclear rather than treating third-party writeups as authoritative.
- Focus on whether the service exposes passwords, hashes, source names, and timestamps because those fields directly affect severity calibration.
- Your project notes require conditional severity logic here. Do not finalize severity without field-level evidence.
- My first-pass severity assessment:
- Preserved note from project docs: conditional severity logic must be preserved exactly.
- Likely reason: breach presence alone may be medium/high, but direct credential material or stronger corroboration may increase severity.
- Status: derive exact trigger conditions from official docs and real example payloads.
