---
title: "prompt / breach / hibp"
aliases: ["hibp", "hibp prompt", "hibp research prompt"]
tags: [zima, research, prompts, provider-research, breach, hibp, graph_exclude]
...
- Provider website / API docs URL: https://haveibeenpwned.com/API/v3
...
- Schema hints already captured in my notes:
  - likely provider method(s): get_breaches(email)
  - likely provider-side category label(s): typed schema; has mapper
  - likely supported entity type(s): email
  - rough provider confidence note from my research scaffold: varies
- Important provider-specific cautions:
- Official API docs are strong and versioned. Research both breach endpoints and stealer-log endpoints separately because they imply different severity outcomes.
- Your implementation notes already say HIBP has a typed schema and existing mapper. Keep research focused on exact field triggers, auth rules, no-hit behavior, and paid-subscription constraints.
- Distinguish traditional breach records from stealer-log domain exposure; they should not collapse into one signal if the fields materially differ.
