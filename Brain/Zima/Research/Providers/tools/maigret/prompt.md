---
title: "prompt / tools / maigret"
aliases: ["maigret", "maigret prompt", "maigret research prompt"]
tags: [zima, research, prompts, provider-research, tools, maigret, graph_exclude]
...
- Provider name: maigret
- Provider category: tools
- Provider website / API docs URL: https://github.com/soxoj/maigret
- Target module(s) from my provider map: `username_exposure`, `account_inventory`
- Build priority / tier: P1 / Core
- Provider role: utility_only or enrichment_only — username-to-service presence discovery feeds account inventory
- Tool type: local Python library / CLI
- Schema hints already captured in my notes:
  - likely provider method(s): search(username) — checks username across 2500+ sites
  - likely provider-side category label(s): username_enumeration, account_discovery, OSINT
  - likely supported entity type(s): username
  - rough provider confidence note from my research scaffold: varies per site — maigret has per-site reliability ratings
- Important provider-specific cautions:
  - Maigret checks username presence across 2500+ sites using HTTP response analysis (status codes, response body content, redirects).
  - It has the largest site database of the username enumeration tools (vs WhatsMyName ~600, Sherlock ~400).
  - Supports JSON output (`--json`), which is the integration path for Zima.
  - Per-site reliability tags exist in the site database — some sites produce false positives. Document how to filter by reliability.
  - Username presence does NOT prove account ownership — it proves the username is taken. Do not overstate.
  - Rate limiting and request delays are important — maigret has built-in rate control. Document the `--timeout` and delay options.
  - Can also extract additional profile data from some sites (profile URL, name, bio) — this is enrichment.
  - Complements holehe (email-based, ~120 sites) and whatsmyname (username-based, ~600 sites). Maigret has the broadest coverage but is slower.
- My first-pass severity assessment:
  - Username presence findings are info — they feed account inventory.
  - The aggregate exposure pattern (username found on 200+ sites) could be a low/medium signal for `username_exposure` module, but individual site hits are not signals.
