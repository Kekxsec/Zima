---
title: "prompt / breach / intelx"
aliases: ["intelx", "intelx prompt", "intelx research prompt"]
tags: [zima, research, prompts, provider-research, breach, intelx, graph_exclude]
...
- Provider name: intelx
- Provider category: breach
- Provider website / API docs URL: https://intelx.io/
- Target module(s) from my provider map: stealer_log_exposure, darkweb_identity_monitor
- Build priority / tier: 1 / Core
- Provider role: signal_producer
- Schema hints already captured in my notes:
  - likely provider method(s): search(email, domain)
  - likely provider-side category label(s): credential_leak, darkweb_mention
  - likely supported entity type(s): email, domain
  - rough provider confidence note from my research scaffold: 0.80
- Important provider-specific cautions:
- IntelX covers many datasets and object types; keep only the endpoints relevant to breach, credential, paste, and dark-web identity use cases in scope.
- Do not let generic search features explode the signal taxonomy. Prefer stable internal concepts backed by explicit result fields.
- Check whether result confidence should vary by source type, preview-vs-full access, and whether the hit is a credential leak versus a mention.
