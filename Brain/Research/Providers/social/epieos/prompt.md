---
title: "prompt / social / epieos"
aliases: ["epieos", "epieos prompt", "epieos research prompt"]
tags: [zima, research, prompts, provider-research, social, epieos, graph_exclude]
...
- Provider name: epieos
- Provider category: social
- Provider website / API docs URL: https://epieos.com/
- Target module(s) from my provider map: username_exposure, alias_correlation
- Build priority / tier: 1 / Core
- Provider role: signal_producer
- Schema hints already captured in my notes:
  - likely provider method(s): lookup(email)
  - likely provider-side category label(s): social_media, identity_exposure
  - likely supported entity type(s): email, username
  - rough provider confidence note from my research scaffold: 0.80
- Important provider-specific cautions:
- Determine whether official API/docs are public or whether the product is primarily interactive. If API access is unclear, document that as an implementation constraint.
- Keep username/alias discovery separate from account-existence risk and from pure enrichment.
- Avoid mapping passive profile discovery to high severity without explicit evidence of abuse or compromise.
