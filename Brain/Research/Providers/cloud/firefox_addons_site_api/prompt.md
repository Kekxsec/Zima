...
- Important provider-specific cautions:
  ...
  - Determine whether Firefox add-on metadata is exposed through official API, embedded JSON, or unsupported scraping paths.
  - Be explicit about maintenance risk if access relies on undocumented endpoints.
  - Assess whether permission, review, author, rating, category, or last-updated metadata can be normalized safely without over-interpreting store presentation data.

---

## Research Instructions

Use official provider documentation, official API references, official OpenAPI specs, official example responses, official site developer references, and provider-maintained source materials as primary sources. Use third-party sources only to fill gaps, and label them clearly.

For every important claim about schema, trigger logic, enums, authentication, limits, access model, response stability, or maintenance expectations, cite the source. Clearly label whether each trigger is:
- documented
...
Because this provider is expected to be enrichment-oriented, do not invent standalone signals unless the available provider data clearly justifies them at the module level.

Identify all relevant endpoints, methods, output artifacts, embedded JSON paths, or other access patterns for this provider and classify each as:
- direct_signal_input
- enrichment_only
- utility_only
- out_of_scope

Important: stay scoped to this provider only. Do not generalize from Chrome Web Store, AMO-adjacent tooling, or unrelated extension intelligence sources unless the official Mozilla materials explicitly reference the same backend or schema.

Specific research focus for this provider:
- determine whether Firefox add-on metadata is available via official API, embedded JSON, or unsupported scraping paths
- document fields available, stability, auth/access model, and rate-limit behavior if documented
- assess whether permissions, review status, ratings, versioning, authorship, categories, or review metadata can be normalized into useful enrichment fields
- distinguish clearly between stable implementation-grade metadata fields and brittle presentation-layer fields
- treat local extension detection as out of scope unless the provider docs explicitly support it
...
For each relevant endpoint, method, output artifact, or metadata access path, document:
- endpoint name/path or artifact name
- purpose
...
- which fields are always present vs optional vs conditional vs undocumented
...
- confirm whether the provider should remain enrichment-only despite exposing risk-relevant metadata
...
If no standalone signals are justified, return no tags for signal rows and explain why.
...
- rate limits, robots/usage constraints, licensing, or undocumented-endpoint risk
...
- what metadata is safe to normalize versus what is too brittle or presentation-derived for production use
...
Provide a structured per-endpoint, per-method, per-artifact, or per-access-path appendix first.
...
- notes = freshness limits, undocumented access risks, schema stability, licensing caveats, and parsing quirks

If there are no standalone signals for this provider, return an empty signal table and explain why in the module mapping table and provider summary.
...
### E. Provider Summary
After the table, add:
1. strongest enrichment uses
2. whether any standalone signal types are actually justified
3. what the provider should not be used for
4. API/auth/rate-limit/licensing/undocumented-access implementation cautions
