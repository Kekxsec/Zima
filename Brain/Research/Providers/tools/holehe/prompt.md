---
title: "prompt / tools / holehe"
aliases: ["holehe", "holehe prompt", "holehe research prompt"]
tags: [zima, research, prompts, provider-research, tools, holehe, graph_exclude]
...
- Provider name: holehe
- Provider category: tools
- Provider website / API docs URL: https://github.com/megadose/holehe
- Target module(s) from my provider map: `account_inventory`, `account_enumeration_risk`
- Build priority / tier: P1 / Core
- Provider role: utility_only or enrichment_only — email-to-service account existence checking feeds account inventory
- Tool type: local Python library / CLI
- Schema hints already captured in my notes:
  - likely provider method(s): check(email) — checks if an email is registered on ~120 websites
  - likely provider-side category label(s): account_discovery, account_enumeration
  - likely supported entity type(s): email
  - rough provider confidence note from my research scaffold: medium-high for positive hits (account exists), but site coverage is limited to ~120 services
- Important provider-specific cautions:
  - Holehe checks if an email is registered on websites by exploiting password reset, login, and registration forms. This means it makes real HTTP requests to third-party services.
  - Rate limiting is critical — too many requests too fast will get blocked or trigger CAPTCHAs.
  - Some services may send "someone tried to reset your password" emails to the target. Document which services have this side effect.
  - Account existence is enrichment for account inventory, not a security finding. The security value comes from cross-referencing discovered accounts with breach data and MFA status.
  - This is a local Python tool (`pip install holehe`), not a hosted API. Can be imported as a library.
  - Complements maigret (username-based) and whatsmyname (username-based). Holehe is email-based, which is more relevant for Zima's primary input (user's email).
- My first-pass severity assessment:
  - Account existence findings are info/low — they feed the account inventory, not standalone signals.
  - The value is in the account inventory, not in individual "account found on X" signals.
