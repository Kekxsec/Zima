---
title: "prompt / tools / chromium_enterprise_policies"
aliases: ["chromium_enterprise_policies", "chromium enterprise policies prompt", "chromium_enterprise_policies research prompt"]
tags: [zima, research, prompts, provider-research, tools, chromium_enterprise_policies, graph_exclude]
...
- Provider name: chromium_enterprise_policies
- Provider category: tools
- Provider website / API docs URL: Chrome Enterprise policy docs
- Documentation / repo URL: Chrome Policy List
...
- Primary browser use case: inspect enforced Chromium-family browser policies for extensions, cookies, and privacy controls
- Browser support: chrome, edge, brave, chromium-based browsers
- Important provider-specific cautions:
  - Useful only when policies are actually present.
  - OS-specific storage differs across registry, plist, and JSON sources.
  - Scope this research to local policy discovery and parsing relevant to browser configuration evidence on managed devices.
  - Distinguish configured policy presence from effective browser behavior where inheritance, precedence, or browser support differences may apply.
  - Do not assume all Chromium-family browsers implement all Chrome Enterprise policy keys identically.
...
Use official browser policy documentation, official policy lists, official enterprise/admin documentation, official Chromium/Chrome/Edge policy references, and provider-maintained documentation as primary sources. Use third-party sources only to fill gaps, and label them clearly.
...
- how policy namespaces/paths differ by browser
...
- how to distinguish mandatory/enforced policy from recommended/default policy where supported
- how to handle policy absence, partial policy presence, and browser-specific unsupported keys
...
Important: stay scoped to Chromium-family enterprise/browser policy sources only. Do not generalize from non-Chromium browser policy systems unless the docs explicitly compare behavior.
...
Document the Chromium-family enterprise policy surface relevant to `browser_configuration`.

At minimum, investigate and document:

**Relevant policy storage / discovery artifacts**
- Windows registry locations for Chrome / Edge / other Chromium-family policy stores, where documented
- macOS plist / managed preferences locations and namespaces
- Linux managed policy JSON locations and formats
- browser-specific policy namespaces, vendor prefixes, or path differences
- any official browser-internal inspection surfaces that are useful for validation only if documented and relevant
- policy precedence and scope, if documented (machine vs user, mandatory vs recommended, etc.)

**Relevant policy families / key groups**
- extension control policies
- extension installation allow/block/force-install controls
- extension developer mode / external install / source restrictions if supported
- cookies / site data / third-party cookie controls
- privacy / tracking / telemetry / browsing data controls, where officially documented for Chromium-family policies
- browser sign-in / sync / password-manager/privacy-adjacent settings only if they map cleanly to `browser_configuration`
...
- supported browser behavior (Chrome / Edge / Brave / Chromium-based browsers)
...
- policy source present but values unsupported by current browser
- common error cases (permission denied, malformed JSON/plist, registry read failure, namespace mismatch)

**Document schema/details important to implementation, including where applicable:**
- browser/vendor identifier
- OS-specific storage path / registry path / plist domain / JSON path
- policy key name
- policy value type
- effective scope (machine vs user, if documented)
- recommended vs mandatory distinction, if documented
- list/dict/scalar value shapes
- extension IDs, URLs, domains, allow/block lists, site patterns, cookie settings, privacy flags
...
- direct configuration evidence around enforced extension, cookie, and privacy-related policies
- which policy keys should remain enrichment-only because they are too generic, low-risk, or too browser-specific for clean normalization
...
Important:
- Most Chromium enterprise policy findings will likely be `low` or `info`, unless a policy directly weakens browser security in a materially meaningful way.
...
- effective-behavior confidence may be lower than configured-policy confidence when browser support or precedence rules are ambiguous
- absence of policy is weaker evidence than presence of explicit policy
- corroboration may include browser internal policy inspection, extension inventory, browser settings inspection, or endpoint telemetry
...
- OS/browser-specific storage differences
- policy precedence / machine-vs-user / mandatory-vs-recommended interpretation issues
- parse safety for registry / plist / JSON artifacts
...
- how to normalize the same policy concept across Chrome, Edge, Brave, and other Chromium-based browsers

Additionally for chromium_enterprise_policies:
- define which browsers Zima should support first
...
- document how to handle list-valued policies such as blocklists, allowlists, and forcelists
...
Provide a structured appendix of relevant Chromium-family policy storage locations, namespaces, key families, value types, and parseability considerations for the target module.
...
If there are no standalone signals for a policy family or browser, return an empty portion of the signal table for that scope and explain why.
...
3. local execution, privilege, storage, or browser-support cautions
4. whether this provider should be treated as signal-producing, enrichment-only, utility-only, deferred, or mixed depending on artifact and browser
