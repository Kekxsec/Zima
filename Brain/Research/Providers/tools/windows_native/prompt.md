---
title: "prompt / tools / windows_native"
aliases: ["windows_native", "windows native prompt", "windows_native research prompt"]
tags: [zima, research, prompts, provider-research, tools, windows_native, graph_exclude]
...
**Provider:** windows_native
**Category:** tools
**URL:** https://learn.microsoft.com — PowerShell and WMI/CIM docs
**Tool type:** os_native_interface — PowerShell, WMI/CIM, native tools
**OS support:** Windows only

**Target modules:** `os_security`, `patch_status`, `disk_encryption_check`, `firewall_status`, `software_vulnerability`
...
**Cautions:**
Output is fragmented across many interfaces. Some require admin. Avoid `Win32_Product` (slow, triggers MSI reconfiguration). Prefer CIM cmdlets over WMI.

**First-pass severity:**
- BitLocker disabled = high
- Windows Firewall disabled = high
...
Document these specific interfaces and their output:

- `Get-BitLockerVolume` — encryption state
  - Properties: VolumeStatus, ProtectionStatus, EncryptionPercentage, MountPoint, VolumeType
  - ProtectionStatus enum: On / Off
  - VolumeStatus enum: FullyEncrypted / EncryptionInProgress / FullyDecrypted / DecryptionInProgress
  - Requires admin / elevated PowerShell
  - Not available on Windows Home editions
- `Get-NetFirewallProfile` — firewall per profile (Domain/Private/Public)
  - Properties: Name, Enabled (True/False), DefaultInboundAction, DefaultOutboundAction
  - Three profiles: Domain, Private, Public — all must be checked
  - Requires admin for full details
- `Get-MpComputerStatus` (Windows Defender) — real-time protection, definitions
  - Properties: RealTimeProtectionEnabled, AntivirusEnabled, AntispywareEnabled, AntivirusSignatureLastUpdated, QuickScanAge, FullScanAge
  - Definition staleness: AntivirusSignatureAge (days)
  - Not available if third-party AV replaces Defender
- `Get-CimInstance Win32_OperatingSystem` — OS version, build
  - Properties: Caption, Version, BuildNumber, LastBootUpTime, OSArchitecture
  - Version string format and how to map to marketing names (Windows 10/11)
- `Get-HotFix` — installed updates
  - Properties: HotFixID, InstalledOn, Description
  - Alternative: `Get-CimInstance Win32_QuickFixEngineering`
  - Limitations: only shows OS hotfixes, not all updates
- Registry: `HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System` — UAC
  - Keys: EnableLUA (0=disabled, 1=enabled), ConsentPromptBehaviorAdmin, ConsentPromptBehaviorUser
  - How to read via PowerShell: `Get-ItemProperty`
- `Get-CimInstance -ClassName CIM_Product` vs `Get-Package` — installed software
  - **DO NOT use Win32_Product** — triggers MSI reconfiguration, extremely slow
  - `Get-Package` from PackageManagement module — preferred approach
  - Properties: Name, Version, ProviderName
  - Alternative: registry enumeration of `HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*`
- `Get-WindowsOptionalFeature -Online` — optional features
  - Properties: FeatureName, State (Enabled/Disabled)
  - Requires admin

For each interface, document:
- exact PowerShell invocation
- output object properties and types
- admin/elevation requirements
- Windows version compatibility (Win 10 vs 11, Home vs Pro vs Enterprise)
...
Additionally:
- Admin requirements: document which commands require elevated PowerShell and what the fallback behavior is when run non-elevated. Determine if Zima should always request elevation or handle both modes.
- PowerShell execution policy: document whether scripts need `Set-ExecutionPolicy` changes or if `-ExecutionPolicy Bypass` per invocation is sufficient.
- Output parsing: PowerShell returns objects, not text. Document serialization strategy (ConvertTo-Json, property access, Format-List) and how the Python client should invoke and parse PowerShell output.
- Windows version differences: document which cmdlets are available on Windows 10 vs 11, Home vs Pro vs Enterprise. Especially note BitLocker (Pro/Enterprise only) and Get-WindowsOptionalFeature availability.
- Document the relationship between this provider and the posture provider — windows_native provides the raw data, posture provides the abstraction layer.
